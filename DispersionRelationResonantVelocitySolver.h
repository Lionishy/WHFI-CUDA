...
namespace iki { namespace whfi {
	...
	std::optional<std::pair<T, T>> DRResonantVelocitySolve(T resonant_v, PhysicalParamenters<T> const &params) {
		auto Zc = Z(resonant_v - params.bulk_to_term_c)
			, Zh = Z(resonant_v * std::sqrt(params.TcTh_ratio) - params.bulk_to_term_h);
		auto dispersion_relation = [&params,resonant_v] (T omega) {
			return T(1. / 1836.) 
		...
		return { (*omega_opt - T(1.)) / (resonant_v * params.betta_root_c) ,*omega_opt };
	}
} /* whfi */ } /* iki */
