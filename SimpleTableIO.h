...
std::ostream& operator<<(std::ostream &ascii_os, iki::UniformSimpleTable<T, Dim, Scale> const &table) {
	...
	Index<Dim> expanded_index = first_index();
	for (auto it = table.data(), end = table.data() + collapsed_size(&talbe.bounds); it != end;) {
		for (size_t arg_idx = 0u; arg_idx != Dim; ++arg_idx)
			ascii_os << uniform_argument(expanded_index[arg_idx],table.space) << ' ';
		for (size_t scale_counter = 0u; scale_counter != Dim; ++scale_counter)
			ascii_os << *it++ << ' ';
		ascii_os << '\n';
		next_index(&expanded_argument, &table.bounds);
	}
